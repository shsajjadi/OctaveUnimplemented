...
namespace image
{
  ...
  {
    static constexpr std::complex<T> infinity ()
    {
...
DEFUN_DLD (graydist, args, nargout,
           R"helpdoc(-*- texinfo -*-
@deftypefn {Loadable Function} {T =} graydist(@var{I}, @var{mask})
...
The return value @var{T} is a gray level image where the value of each point corresponds
 to its shortest distance to the provided seed points. If 'P' is the path between points 'a' and 'b'
the gray weighted distance corresponds to the surface area estimation under the path[1].

The cost of each step in GWD is computed as:@*

@code{0.5 * (I(p) + I(q)) * SpatialDistance(pq)}@*
